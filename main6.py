...
import cv2
import numpy as np
from scipy.optimize import linear_sum_assignment # for Hungarian algo
import time
import os
...
script_dir = Path(__file__).resolve().parent # get parent of current dir

video_path = r"IMG_8290.mp4"
video_basename = os.path.splitext(os.path.basename(video_path))[0] # extract IMG_8290.mp4
output_dir = "output_videos"
output_path = os.path.join(output_dir, f"{video_basename}_output.mp4")

# Create output directory if it doesn't exist
os.makedirs(output_dir, exist_ok=True) # will not raise an error if the dir already  exists
...
def calculate_cost_matrix(tracked_objects, detections):
    """Calculates the cost of matching each detection to tracked object for associating objects in current frame with previous frames."""
    num_tracks = len(tracked_objects)  #dict
    num_detections = len(detections) #list

    cost_matrix = np.zeros((num_tracks, num_detections), dtype=np.float32)

    for i, track_data in enumerate(tracked_objects.values()): # for each tracked obj
        for j, detection in enumerate(detections): # for each detection
            bbox = detection[:4]
...
def calculate_iou(tracked_bb, new_BB):
    """Calculates the Intersection over Union (IoU) of two bounding boxes."""
    x1, y1, x2, y2 = tracked_bb
    x1_p, y1_p, x2_p, y2_p = new_BB
...
def get_movement_direction(frame_width, frame_height, bbox):
    bbheight = abs(bbox[3] - bbox[1])
    ...
    if center_x < left_zone:
        ...
        return "RIGHT"
    elif frame_height - bbheight < 20:
        return "Stop! Subject is too close"
    else:
        return "STRAIGHT"
...
while True:
    ...
    for detection in results[0].boxes:
        x1, y1, x2, y2 = map(int, detection.xyxy[0]) # Bounding box coordinates , top-left and bottom-right
    ...
    # Subject tracking
    if subject_id is None: # no sub is being tracked
        ...
        if frame_count >= 390:
            ...
            # find the smallest distance( Eucl between center of the person and frame center
            ...
            if closest_person:
                ...
                tracked_objects[subject_id] = {'bbox': closest_person[:4], 'age': 0} # age : for duration of the subject
                                                                                    # being tracked
                next_object_id += 1
    ...
    if subject_id in tracked_objects:
        ...
        movement = get_movement_direction(w,h, bbox)
